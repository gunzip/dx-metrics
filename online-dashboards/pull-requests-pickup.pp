...
dashboard "github_repository_pr_pickup_time" {
  title = "Pull Requests Pickup Time (real time)"
  ...
  ##########################################
    ...
    chart {
      title = "PR Pickup Time (moving average)"
      type  = "line"
      width = 12
      ...
      sql = <<EOQ
        WITH time_series AS (
            SELECT generate_series(
                (SELECT MIN((p.created_at)::date) 
                FROM github_pull_request AS p
                WHERE p.repository_full_name = $1
                  AND (p.created_at)::date >= NOW() - CAST($2 AS interval)),
                CURRENT_DATE,
                '1 day'::interval
            )::date AS date
        ),
        pr_lead_times_v1 AS (
          ...
          FROM (
              ...
              WHERE
                  ...
                  AND p.created_at::timestamp >= NOW() - CAST($2 AS interval)
          ) t
          WHERE t.rn = 1
        ),
        pr_lead_times AS (
        ...
        SELECT             
          ...
          FROM time_series t
          INNER JOIN pr_lead_times p ON (p.created_at::date <= t.date)
